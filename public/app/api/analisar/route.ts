...
const FAST_MODEL_TIMEOUT_MS = 34_000;
...
async function chamarModelo(
  ...
  const maxTokens = options.preferFast ? 1_050 : 1_250;
...
function respostaHeuristicaProfissional(params: {
  textoOriginal: string;
  modo: ModoAnalise;
  assuntoDinamico: boolean;
  contextoRSS: string;
  ...
  if (params.spaceNews) return respostaSeguraDeFallback(params.spaceNews);

  const normalizado = normalizarComparacao(params.textoOriginal);
  const alegacaoMedicaPessoal = /\b(aids|hiv|cancer|câncer|doenca|doença|diagnostico|diagnóstico|hospitalizad|internad|morreu|gravida|grávida)\b/i.test(normalizado);
  const pessoaOuCargo = /\b(bolsonaro|lula|virginia|virgínia|vinicius|vinícius|presidente|ex presidente|influenciador|jogador|cantor|atriz|ator|politico|político)\b/i.test(normalizado);
  const rumorRelacional = pareceAlegacaoDeRelacionamentoAtual(params.textoOriginal);
  const rssOk = rssTemResultadosRecentes(params.contextoRSS);

  if (alegacaoMedicaPessoal && pessoaOuCargo) {
    return formatarResultado({
      idioma: "pt-BR",
      classificacao: "Suspeita",
      resumo: "A afirmação envolve dado pessoal de saúde e não apresenta evidência verificável suficiente para ser tratada como fato.",
      analise: [
        "Alegações sobre diagnóstico ou condição médica de uma pessoa pública exigem fonte primária, boletim oficial, declaração verificável ou cobertura jornalística responsável.",
        "Sem esse tipo de prova, a informação deve ser tratada como boato potencialmente difamatório, mesmo quando circula em redes sociais.",
        "O sistema evitou transformar ausência de evidência em confirmação ou negar o fato com base apenas em memória antiga.",
      ],
      sinais: [
        "Tema sensível e pessoal.",
        "Falta de fonte primária ou documento verificável na entrada enviada.",
        "Potencial de dano à reputação se compartilhado sem prova.",
      ],
      recomendacao: "Não compartilhe como verdade. Procure confirmação em fontes oficiais, boletins públicos ou veículos jornalísticos confiáveis antes de concluir.",
      observacao: "Esta é uma análise conservadora de segurança quando a resposta automática principal veio fora do padrão.",
    });
  }

  if (rumorRelacional && !rssOk) {
    return formatarResultado({
      idioma: "pt-BR",
      classificacao: "Não Confirmado",
      resumo: "Não há evidência suficiente, na entrada analisada, para confirmar o suposto relacionamento ou envolvimento citado.",
      analise: [
        "Relacionamentos, separações e rumores envolvendo pessoas públicas mudam com o tempo e exigem fontes atuais.",
        "Sem declaração das partes, registro verificável ou notícia recente confiável, a conclusão responsável é não confirmar a alegação.",
      ],
      sinais: ["Afirmação de vida pessoal sem prova apresentada.", "Assunto sujeito a boatos e especulação em redes sociais."],
      recomendacao: "Aguarde fontes atuais e verificáveis antes de tratar a informação como verdadeira ou falsa.",
      observacao: "O sistema evita usar estado civil antigo ou memória interna como prova atual.",
    });
  }
...
async function gerarAnaliseConfiavel(params: {
  ...
  const maxTentativas = params.preferFast ? 2 : 3;
  ...
  for (let tentativa = 1; tentativa <= maxTentativas; tentativa += 1) {
    try {
      ...
      ultimoMotivo = (validacao as { ok: false; motivo: string }).motivo;
      console.warn(
  ...
  return respostaHeuristicaProfissional({
    textoOriginal: params.textoOriginal,
    modo: params.modo,
    assuntoDinamico: params.assuntoDinamico,
    contextoRSS: params.contextoRSS,
    spaceNews: params.spaceNews,
  });
}
...
export async function POST(req: Request) {
  try {
    ...
    const effectiveType = String(data.clientProfile?.effectiveType || "").toLowerCase();
    const preferFast =
